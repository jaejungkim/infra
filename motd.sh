...
cert_expiry=$(  LC_TIME="en_US.UTF-8" curl -v --connect-timeout 5 --max-time 5 https://tt.inclunet.com 2>&1 | grep -i "expire date" | awk '{print substr($0, index($0,$4))}' | xargs -I {} date -d "{}" +"%s")
